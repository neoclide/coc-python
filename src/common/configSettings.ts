...
export class PythonSettings implements IPythonSettings {
  ...
  public disableLanguageServer = false
  ...
  protected update(pythonSettings: WorkspaceConfiguration) {
    ...
    this.disableLanguageServer = systemVariables.resolveAny(pythonSettings.get<boolean>('disableLanguageServer', false))!
    if (this.jediEnabled) {
      // tslint:disable-next-line:no-backbone-get-set-outside-model no-non-null-assertion
  ...
  protected onWorkspaceFoldersChanged() {
    ...
    const workspaceKeys = this.workspace.workspaceFolders!.map(workspaceFolder => Uri.parse(workspaceFolder.uri).fsPath)
