...
export abstract class BaseInstaller {
  ...
  public promptToInstall(product: Product, resource?: Uri): Promise<InstallerResponse> {
    ...
    const key = `${product}${workspaceFolder ? Uri.parse(workspaceFolder.uri).fsPath : ''
      }`
...
export class CTagsInstaller extends BaseInstaller {
  ...
  public async install(_product: Product, _resource?: Uri): Promise<InstallerResponse> {
    if (this.serviceContainer.get<IPlatformService>(IPlatformService).isWindows) {
      ...
      let res = await workspace.runTerminalCommand(CTagsInsllationScript, process.cwd(), true)
      if (!res.success) {
        const logger = this.serviceContainer.get<ILogger>(ILogger)
  ...
  protected async promptToInstallImplementation(product: Product, resource?: Uri): Promise<InstallerResponse> {
    const res = await workspace.showPrompt('Install CTags to enable Python workspace symbols?')
    return res ? this.install(product, resource) : InstallerResponse.Ignore
  }
}
...
export class FormatterInstaller extends BaseInstaller {
  protected async promptToInstallImplementation(product: Product, resource?: Uri): Promise<InstallerResponse> {
    ...
    const idx = await workspace.showQuickpick(options, message)
    if (idx !== -1) {
  ...
  protected async promptToInstallImplementation(product: Product, resource?: Uri): Promise<InstallerResponse> {
    ...
    if (idx !== -1) {
      ...
      if (response === selectLinter) {
        ...
        commandManager.executeCommand(Commands.Set_Linter)
      }
    }
    return InstallerResponse.Ignore
  }
...
export class TestFrameworkInstaller extends BaseInstaller {
  protected async promptToInstallImplementation(product: Product, resource?: Uri): Promise<InstallerResponse> {
    ...
    const idx = await workspace.showQuickpick(options, message)
    return idx !== -1 && options[idx] == 'Yes' ? this.install(product, resource) : InstallerResponse.Ignore
  }
}
...
export class RefactoringLibraryInstaller extends BaseInstaller {
  protected async promptToInstallImplementation(product: Product, resource?: Uri): Promise<InstallerResponse> {
    const productName = ProductNames.get(product)!
    const res = await workspace.showPrompt(`Refactoring library ${productName} is not installed. Install?`)
    return res ? this.install(product, resource) : InstallerResponse.Ignore
  }
}
