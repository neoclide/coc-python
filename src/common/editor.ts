  ...
  filePatches.forEach(patch => {
    ...
    patches.forEach(p => {
      ...
      getTextEditsInternal(fileSource, p.diffs, p.start1).forEach(edit => {
        switch (edit.action) {
          case EditAction.Delete:
            edits.push(TextEdit.del(Range.create(edit.start, edit.end)))
            break
          case EditAction.Insert:
            edits.push(TextEdit.insert(edit.start, edit.text))
            break
          case EditAction.Replace:
            edits.push(TextEdit.replace(Range.create(edit.start, edit.end), edit.text))
            break
          default:
            break
        }
