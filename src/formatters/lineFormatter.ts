...
export class LineFormatter {
  ...
  public formatLine(document: TextDocument, lineNumber: number): string {
    this.document = workspace.getDocument(document.uri)
    this.lineNumber = lineNumber
    this.text = workspace.getDocument(document.uri).getline(lineNumber)
  ...
  private isInsideFunctionArguments(position: number): boolean {
    ...
    for (let i = this.lineNumber; i >= 0; i -= 1) {
      ...
      const lineTokens = new Tokenizer().tokenize(line)
    ...
    const text = this.document.textDocument.getText(Range.create(start, Position.create(this.lineNumber, currentLine.length)))
    ...
    position = this.document.textDocument.offsetAt(Position.create(this.lineNumber, 0)) + position - this.document.textDocument.offsetAt(start)
  ...
  private getPreviousLineTokens(): ITextRangeCollection<IToken> | undefined {
    ...
    return new Tokenizer().tokenize(line)
  }
}
