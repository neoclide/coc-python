...
export class PythonSignatureProvider implements SignatureHelpProvider {
  ...
  private static parseData(data: proxy.IArgumentsResult): SignatureHelp | null {
  ...
  public provideSignatureHelp(document: TextDocument, position: Position, token: CancellationToken): Thenable<SignatureHelp | null> {
    ...
    return this.jediFactory.getJediProxyHandler<proxy.IArgumentsResult>(Uri.parse(document.uri)).sendCommand(cmd, token).then(data => {
